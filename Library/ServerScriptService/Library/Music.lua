...
-- Override classes to detect changes
ChangedTracker.ApplyToClassWithEvent(Music, {"SetEnabled", "SetActivePlaylist", "RemoveCustomPlaylistTrack"})
ChangedTracker.ApplyToClassWithEvent(Music.Playlist, {"SetName", "SetSong", "RemoveSong"})
local base = Music.CreateNewPlaylist
function Music:CreateNewPlaylist(...)
	local success, playlist = base(self, ...)
	if success then
		self:RecordChanged()
	end
	return success, playlist
end
local base = Music.RecordSaved
function Music:RecordSaved(saveTime)
	base(self, saveTime)
	for _, playlist in self.customPlaylists do
		playlist:RecordSaved(saveTime)
	end
end
local base = Music.HasChanges
function Music:HasChanges()
	if base(self) then return true end
	for _, playlist in self.customPlaylists do
		if playlist:HasChanges() then return true end
	end
end
...
function Music.InitRemotes(newRemote)
	Music.Playlist.UpdatePlaylist = newRemote:Event("UpdatePlaylist")
	...
	newRemote:Function("CreateCustomPlaylist", function(player, music, data)
		...
		data = data or {}
		local success, playlistOrProblem = music:CreateNewPlaylist(data.Name and validatePlaylistName(player, data.Name), Music.FilterSongsAsync(data.Songs or {}))
		if success then
